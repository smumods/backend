namespace :db do
  ...
  task id_to_uuid: :environment do
    ...
    table_names = ActiveRecord::Base.connection.tables - ["schema_migrations", "ar_internal_metadata", "migration_validators", "ar_internal_metadata", "friendly_id_slugs", "active_admin_comments", "active_storage_blobs", "active_storage_attachments"]
  ...
  desc "TODO"
  task actions_to_uuid: :environment do
    puts "Setting Action UUIDs"
    Action.all.each do |action|
      action_target_uuid = action.target.uuid
      action_user_uuid = action.user.uuid
      action.update(target_uuid: action_target_uuid, user_uuid: action_user_uuid)
    end
  end

  task finalize_uuids: :environment do
    puts "Finalizing UUIDs"
    table_names = ActiveRecord::Base.connection.tables - ["schema_migrations", "ar_internal_metadata", "migration_validators", "ar_internal_metadata", "friendly_id_slugs", "active_admin_comments", "active_storage_blobs", "active_storage_attachments"]
